...
class TestOGCheckScoring:
    def test_basic_score_in_range(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 100.0, "hour": 14})
        assert isinstance(result, OGCheckResult)
        assert 0.0 <= result.fraud_score <= 1.0

    def test_result_fields(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 100.0, "hour": 14})
...
class TestOGCheckRules:
    def test_high_amount_fires(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 5000.0, "hour": 14})
        names = [v.rule_name for v in result.violations]
        assert "HIGH_AMOUNT" in names

    def test_max_amount_fires(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 15000.0, "hour": 14})
        names = [v.rule_name for v in result.violations]
        assert "MAX_AMOUNT" in names
        assert "HIGH_AMOUNT" in names  # should also fire

    def test_late_night_fires(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 100.0, "hour": 3})
        names = [v.rule_name for v in result.violations]
        assert "LATE_NIGHT" in names

    def test_round_amount_fires(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 1000.0, "hour": 14})
        names = [v.rule_name for v in result.violations]
        assert "ROUND_AMOUNT" in names

    def test_micro_amount_fires(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 0.50, "hour": 14})
        names = [v.rule_name for v in result.violations]
        assert "MICRO_AMOUNT" in names

    def test_velocity_fires(self, og_check: OGCheck):
        result = og_check.analyze({
    ...
    def test_email_mismatch_fires(self, og_check: OGCheck):
        result = og_check.analyze({
            "TransactionAmt": 100.0, "hour": 14,
    ...
    def test_disposable_email_fires(self, og_check: OGCheck):
        result = og_check.analyze({
            "TransactionAmt": 100.0, "hour": 14,
    ...
    def test_no_device_high_value_fires(self, og_check: OGCheck):
        result = og_check.analyze({
            "TransactionAmt": 800.0, "hour": 14,
            "DeviceInfo": "",
        })
        names = [v.rule_name for v in result.violations]
        assert "NO_DEVICE_HIGH_VALUE" in names

    def test_bot_device_fires(self, og_check: OGCheck):
        result = og_check.analyze({
            "TransactionAmt": 100.0, "hour": 14,
            "DeviceInfo": "headless-chrome v99",
        })
        names = [v.rule_name for v in result.violations]
        assert "BOT_DEVICE" in names

    def test_clean_transaction_no_violations(self, og_check: OGCheck):
        result = og_check.analyze({
    ...
    def test_weekend_high_value_fires(self, og_check: OGCheck):
        result = og_check.analyze({
            "TransactionAmt": 2000.0, "hour": 14, "is_weekend": 1,
        })
        names = [v.rule_name for v in result.violations]
...
class TestRuleViolation:
    def test_severity_values(self, og_check: OGCheck):
        result = og_check.analyze({"TransactionAmt": 15000.0, "hour": 3})
